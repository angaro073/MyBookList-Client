...
console.log(filters);
//
        axios.get(`https://www.googleapis.com/books/v1/volumes?${filters}`)
        .then(
          (response) => {
...
//
      this.inSearchProcess = true;
      axios.get(`https://www.googleapis.com/books/v1/volumes/${bookId}?fields=id,volumeInfo(title,industryIdentifiers,authors,publisher,publishedDate,description,pageCount,categories,imageLinks,language),saleInfo/buyLink`)
      .then(
        (response) => {
          this.inSearchProcess = false;
//
